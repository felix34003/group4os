...
6. **Prioritize Persistence**  
   Always save plans to a dedicated `planning/` directory as Markdown files with descriptive names. This ensures plans are searchable and persistent across sessions.
...
- **Skill Discovery**: Identify existing Agent Skills (in `.agents/skills/`) that can be applied to automate or simplify the implementation steps.
...
- **Skill Application Map**: Mapping specific steps to available agent skills (e.g., `use-wsl`, `github`, `log-changes`)
- Verification steps

Plans should be **scannable but precise**. Any plan created should be saved to the `planning/` directory in the project root.
...
A list of available skills and how they will be used to execute specific parts of the plan:
- **{Skill Name}**: {Application description}
- **{Skill Name}**: {Application description}
